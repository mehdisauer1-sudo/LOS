...
const INIT_PENDING = []; // aucune demande au départ
const INIT_WEAPONS = []; // aucune arme au départ
const INIT_TOOLS = []; // aucun outil au départ
const INIT_COTAS = []; // aucun cota au départ
const INIT_COMPTA = []; // aucune transaction au départ
const INIT_RAPPORTS = []; // aucun rapport au départ
const INIT_CLIPS = []; // aucun clip au départ
...
function LoginPage({
  ...
  const doLogin = () => {
    ...
    if (acc.banned) {
      setError("Votre compte a été banni de l'organisation.");
      return;
    }
    if (!acc.approved) {
      setError(
  ...
  const doRegister = () => {
    if (!form.username || !form.password || !form.nomRP || !form.prenomRP) {
      setError("Tous les champs sont obligatoires.");
      return;
    }
    if (
      accounts.find((a) => a.username === form.username) ||
      pendingAccounts.find((a) => a.username === form.username)
    ) {
    ...
    setError("");
    setForm({ username: "", password: "", nomRP: "", prenomRP: "" });
  };
      ...
      >
        ...
        {mode === "login" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <Btn onClick={doLogin} color={T.orange} full>
              🔐 Se connecter
            </Btn>
          </div>
        )}
        ...
        {mode === "register" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            >
              <Field label="Prénom RP">
                <Inp
                  value={form.prenomRP}
                  onChange={(e) =>
                    setForm((p) => ({ ...p, prenomRP: e.target.value }))
                  }
              ...
              <Field label="Nom RP">
                <Inp
                  value={form.nomRP}
                  onChange={(e) =>
                    setForm((p) => ({ ...p, nomRP: e.target.value }))
                  }
            ...
            <Btn onClick={doRegister} color={T.orange} full>
              📩 Envoyer la demande
            </Btn>
          </div>
        )}
      ...
      >
        <div
          style={{
            ...
            fontSize: 34,
            marginBottom: 10,
            boxShadow: `0 0 20px ${T.orange}11`,
          }}
        >
          🦂
        </div>
    ...
    >
      <div style={{ flex: 1 }}>
        <div style={{ color: "#fff", fontWeight: 900, fontSize: 15 }}>
          LOS MALDITAS <span style={{ color: T.orange }}>14</span>
        </div>
      ...
      >
        ...
        <div>
          ...
          <div style={{ color: T.green, fontWeight: 800, fontSize: 14 }}>
            {fmt(fondsOrg)} $
          </div>
        </div>
      </div>
...
function PageAccueil({ accounts, weapons, tools, cotas, compta, radio }) {
  ...
  const fondsOrg = compta.reduce(
    (a, c) => a + (c.type === "credit" ? c.montant : -c.montant),
    0
  );
  const gains = compta
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      >
        <div
          style={{
            flex: 1,
            background: "linear-gradient(135deg,#1a0800,#0e0e0e)",
            padding: "28px 32px",
            display: "flex",
            flexDirection: "column",
        ...
        >
          <div style={{ display: "flex", gap: 12 }}>
            ...
            >
              ...
              <span style={{ color: T.yellow, fontSize: 12, fontWeight: 700 }}>
                ACCÈS SÉCURISÉ
              </span>
            </div>
          </div>
          ...
          >
            {[
              ...
              { icon: "🔫", val: weapons.length, label: "Armes" },
              {
                icon: "$",
                val:
                  fondsOrg >= 0 ? `${(fondsOrg / 1000000).toFixed(2)}M $` : "—",
                label: "Fonds Org.",
              },
        ...
        >
          <div style={{ textAlign: "right" }}>
            ...
            <div style={{ color: T.muted, fontSize: 11 }}>
              {t.getDate()} mai {t.getFullYear()}
            </div>
          </div>
          <div
            style={{ fontSize: 68, filter: "drop-shadow(0 0 30px #f9731644)" }}
          >
            🤠
          </div>
        </div>
      ...
      >
        {[
          ...
          },
          {
            icon: "🔫",
            val: weapons.length,
            label: "Armes attribuées",
            sub: `${weapons.length} enreg.`,
            sc: T.orange,
          },
          {
            icon: "🔧",
            val: tools.length,
            label: "Outils actifs",
            sub: `${tools.length} distribués`,
            sc: T.purple,
          },
          {
            icon: "📻",
            val: radio,
            label: "Fréquence radio",
            sub: "MHz · Canal Actif",
            sc: T.cyan,
          },
          {
            ...
            val:
              fondsOrg >= 0
                ? `${(fondsOrg / 1000000).toFixed(2)}M $`
                : "-" + fmt(Math.abs(fondsOrg)) + " $",
            label: "Fonds organisation",
            sub: `Gains: ${fmt(gains)} $`,
            sc: fondsOrg >= 0 ? T.green : T.red,
          },
        ].map((s, i) => (
          <div
            key={i}
          ...
          {compta.length === 0 ? (
            ...
            >
              ...
              >
                ...
                <div style={{ color: T.green, fontWeight: 900, fontSize: 20 }}>
                  +{fmt(gains)} $
                </div>
              </div>
              <div
              ...
              >
                ...
                <div
                  style={{
                    ...
                    height: 8,
                    borderRadius: "50%",
                    background: m.online ? T.green : "#444",
                  }}
                />
              </div>
            ))
...
function PageClassement({ accounts, cotas }) {
  ...
  const membres = accounts.filter((a) => a.approved && !a.banned);
  const players = membres
    .map((a) => {
      const myCot = cotas.filter((c) => c.joueur === a.nomRP);
      const pts = myCot.reduce(
        (s, c) => s + (c.status === "validée" ? c.pts : 0),
        0
      );
  ...
  const gradeDist = GRADES.map((g) => ({
    grade: g,
    count: membres.filter((m) => m.grade === g).length,
  }));
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      >
        <div>
          <div style={{ color: T.text, fontWeight: 900, fontSize: 24 }}>
            Classement Cotas
          </div>
          <div style={{ color: T.muted, fontSize: 13 }}>
            Points hebdomadaires · Reset chaque lundi
          </div>
        </div>
        <div style={{ display: "flex", gap: 10 }}>
          ...
          >
            ...
            <span style={{ color: T.yellow, fontWeight: 700, fontSize: 13 }}>
              S20 · {t.getDate()} Mai {t.getFullYear()}
            </span>
          </div>
        </div>
      </div>
              ...
              {p2 ? (
                <>
                  ...
                  >
                    ...
                    <div
                      style={{ color: T.muted, fontWeight: 800, fontSize: 14 }}
                    >
                      #2
                    </div>
                  </div>
                ...
                <div style={{ color: T.muted, fontSize: 12, padding: 20 }}>
                  —
                </div>
              )}
            </div>
            ...
            >
              {p1 ? (
                <>
                  ...
                  <div
                    style={{ color: T.orange, fontSize: 12, fontWeight: 600 }}
                  >
                    {p1.grade}
                  </div>
                  <div
                  ...
                  >
                    <div
                      style={{ fontSize: 26, marginBottom: 4, color: T.yellow }}
                    >
                      🏆
                    </div>
                    <div
                      style={{ color: T.yellow, fontWeight: 800, fontSize: 16 }}
                    >
                      #1
                    </div>
                  </div>
        ...
        {[
          ...
          <button
            ...
            style={{
              padding: "9px 18px",
              borderRadius: 8,
              border: `1px solid ${tab === id ? T.orange : T.border}`,
              background: tab === id ? T.orange + "22" : "transparent",
              color: tab === id ? T.orange : T.muted,
              fontWeight: 700,
              fontSize: 13,
              cursor: "pointer",
            }}
        ...
        <Panel>
          ...
          >
            <div style={{ color: T.text, fontWeight: 700, fontSize: 14 }}>
              Classement · {t.getDate()} Mai {t.getFullYear()}
            </div>
            {avg > 0 && (
              <Tag text={`📊 Moyenne groupe : ${avg} pts`} color={T.yellow} />
            )}
          </div>
          {players.length === 0 ? (
            ...
            players.map((p, i) => {
              const barW =
                p.pts > 0
                  ? Math.min(
                      100,
                      Math.round((p.pts / (players[0]?.pts || 1)) * 100)
                    )
                  : 0;
              ...
              return (
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div
                      style={{
                        color: GRADE_COLORS[p.grade] || T.muted,
                        fontSize: 12,
                      }}
                    >
                      {p.grade}
                    ...
                    >
                      <div
                        style={{
                          ...
                          background:
                            i === 0
                              ? `linear-gradient(90deg,${T.yellow},${T.orange})`
                              : T.orange + "88",
                          borderRadius: 4,
                  ...
                  <div style={{ textAlign: "right" }}>
                    <div
                      style={{ color: T.yellow, fontWeight: 900, fontSize: 18 }}
                    >
                      {p.pts}
                    </div>
                    <div style={{ color: T.muted, fontSize: 12 }}>
          ...
          >
            ...
            {gradeDist.map((g) => {
              const pct = membres.length
                ? Math.round((g.count / membres.length) * 100)
                : 0;
              return (
                <div key={g.grade} style={{ marginBottom: 12 }}>
                  ...
                  >
                    <span
                      style={{
                        color: GRADE_COLORS[g.grade] || T.muted,
                        fontWeight: 600,
                        fontSize: 13,
                      }}
                    >
                  ...
                  >
                    <div
                      style={{
                        width: `${pct}%`,
                        height: "100%",
                        background: GRADE_COLORS[g.grade] || T.muted,
                        borderRadius: 4,
                      }}
                    />
                  </div>
...
function PageCotas({ cotas, setCotas, me }) {
  ...
  const totalVal = cotas
    .filter((c) => c.status === "validée")
    .reduce((a, c) => a + c.montant, 0);
  ...
  const decide = (id, status) =>
    setCotas((p) => p.map((c) => (c.id === id ? { ...c, status } : c)));
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      >
        <div>
          ...
          <div style={{ color: T.muted, fontSize: 13 }}>
            Soumission et gestion des cotas d'organisation
          </div>
        </div>
        <div style={{ display: "flex", gap: 10 }}>
      ...
      <div style={{ display: "flex", gap: 4 }}>
        <button
          ...
          style={{
            padding: "9px 18px",
            borderRadius: 8,
            border: `1px solid ${tab === "soumettre" ? T.border2 : T.border}`,
            background: tab === "soumettre" ? "#1f1f1f" : "transparent",
            color: tab === "soumettre" ? T.text : T.muted,
            fontWeight: 600,
            fontSize: 13,
            cursor: "pointer",
          }}
        ...
        {me.isAdmin && (
          <button
            ...
            style={{
              padding: "9px 18px",
              borderRadius: 8,
              border: `1px solid ${tab === "admin" ? T.orange : T.border}`,
              background: tab === "admin" ? T.orange + "22" : "transparent",
              color: tab === "admin" ? T.orange : T.muted,
              fontWeight: 700,
              fontSize: 13,
              cursor: "pointer",
              display: "flex",
      ...
      {tab === "soumettre" && (
        <Panel>
          ...
          >
            ...
            <Field label="Matériel utilisé">
              <Inp
                value={form.materiel}
                onChange={(e) =>
                  setForm((p) => ({ ...p, materiel: e.target.value }))
                }
                placeholder="Ex: Kit Drogue x2"
              />
            </Field>
            <Field label="Preuve photo / vidéo">
              <div
                ...
                style={{
                  background: "#0f0f0f",
                  border: `1px dashed ${form.proof ? T.green : T.border2}`,
                  borderRadius: 8,
                  padding: "10px 14px",
                  cursor: "pointer",
                  color: form.proof ? T.green : T.muted,
                  fontSize: 13,
                }}
              >
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (f) setForm((p) => ({ ...p, proof: f.name, file: f }));
                  }}
                />
              </div>
            </Field>
            <Field label="Description" style={{ gridColumn: "1/-1" }}>
              <textarea
                ...
                placeholder="Décrivez le cota : lieu, déroulement, résultat…"
                style={{
                  background: "#0f0f0f",
                  border: `1px solid ${T.border2}`,
                  color: T.text,
          ...
          >
            <Btn
              onClick={submit}
              disabled={!form.type || !form.montant || !form.desc}
              color={ok ? T.green : T.orange}
            >
              {ok ? "✓ Soumis !" : "📤 Soumettre"}
            </Btn>
            {(!form.type || !form.montant || !form.desc) && (
              <span style={{ color: T.muted, fontSize: 12 }}>
      ...
      >
        {cotas.filter((c) => tab === "admin" || c.joueur === me.nomRP)
          .length === 0 ? (
          <div style={{ gridColumn: "1/-1" }}>
            <Empty
              ...
              text={
                tab === "admin"
                  ? "Aucun cota soumis par les membres."
                  : "Vous n'avez pas encore soumis de cota."
              }
            />
          </div>
        ) : (
          cotas
            .filter((c) => tab === "admin" || c.joueur === me.nomRP)
            .map((c) => {
              const col =
                c.status === "validée"
                  ? T.green
                ...
                >
                  <div
                    style={{
                      ...
                      display: "flex",
                      alignItems: "flex-end",
                      backgroundImage: c.proofDataUrl
                        ? `url(${c.proofDataUrl})`
                        : undefined,
                  ...
                  >
                    ...
                    <div style={{ position: "absolute", top: 10, right: 10 }}>
                      <Tag text={c.type} color={T.orange} />
                    </div>
                    <div
                      style={{
                        padding: "8px 14px",
                  ...
                  <div style={{ padding: "12px" }}>
                    ...
                    >
                      <Av
                        init={c.init}
                        size={28}
                        color={GRADE_COLORS[c.grade] || "#374151"}
                        square
                      />
                      <div>
                        <div
                    ...
                    {c.proof && (
                      <div
                        style={{ color: "#444", fontSize: 11, marginBottom: 8 }}
                      >
                        📎 {c.proof}
                      </div>
                    )}
                    ...
                    >
                      ...
                      {(me.isAdmin || c.joueur === me.nomRP) && (
                        <Btn onClick={() => del(c.id)} color={T.red} small>
                          🗑
                        </Btn>
                      )}
                    </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div>
        ...
        <div style={{ color: T.muted, fontSize: 13 }}>
          Upload de preuves, clips et documents de cotas
        </div>
      </div>
      <Panel>
        <Field label="Titre du clip / preuve">
          <Inp
            ...
            placeholder="Ex: Cota #150 - Braquage Banque"
          />
        </Field>
        <div
          onClick={() => fileRef.current.click()}
          style={{
            marginTop: 12,
            border: `1px dashed ${file ? T.green : T.border2}`,
            borderRadius: 12,
            padding: "36px",
            textAlign: "center",
            cursor: "pointer",
        ...
        >
          ...
          <div style={{ color: T.muted, fontSize: 12, marginTop: 4 }}>
            Vidéo, Image, Preuve — Max 500 MB
          </div>
          {file && (
            <div style={{ color: T.green, fontWeight: 600, marginTop: 8 }}>
              ✓ {file.name}
            </div>
          )}
          <input
        ...
        <div style={{ marginTop: 14 }}>
          <Btn onClick={deposit} disabled={!titre || !file} color={T.cyan}>
            ☁ Déposer le Clip
          </Btn>
        </div>
      </Panel>
            ...
            {clips.map((cl) => (
              ...
              >
                ...
                >
                  <Tag
                    text={cl.type}
                    color={cl.type === "VIDEO" ? T.cyan : T.orange}
                  />
                </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div>
        ...
        <div style={{ color: T.muted, fontSize: 13 }}>
          Signalement confidentiel — Visible uniquement par les admins
        </div>
      </div>
      <Panel>
        <div
        ...
        <Field label="Description de l'incident">
          <textarea
            ...
            maxLength={500}
            placeholder="Décrivez l'incident en détail : lieu, heure, personnes impliquées, déroulement…"
            style={{
              background: "#0f0f0f",
              border: `1px solid ${T.border2}`,
              color: T.text,
          ...
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <div style={{ color: T.muted, fontSize: 12 }}>
              Rapport confidentiel — lecture admin uniquement
            </div>
            <div style={{ color: T.muted, fontSize: 12 }}>
              {desc.length}/500
            </div>
      ...
      <div>
        ...
        {(me.isAdmin ? rapports : rapports.filter((r) => r.auteur === me.nomRP))
          .length === 0 ? (
          <Empty icon="📋" text="Aucun rapport." />
        ) : (
          (me.isAdmin
            ? rapports
            : rapports.filter((r) => r.auteur === me.nomRP)
          ).map((r) => (
            <div
              key={r.id}
              style={{
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ color: T.muted2, fontSize: 13, lineHeight: 1.6 }}>
                  {r.texte}
                </div>
              </div>
              <div
              ...
              >
                <Tag
                  text={r.lu ? "Lu" : "Incident"}
                  color={r.lu ? T.muted : T.red}
                />
                {me.isAdmin && !r.lu && (
                  <button
                    onClick={() =>
                      setRapports((p) =>
                        p.map((x) => (x.id === r.id ? { ...x, lu: true } : x))
                      )
                    }
                    style={{
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div>
        ...
        <div style={{ color: T.muted, fontSize: 13 }}>
          Canal de communication sécurisé — Los Malditas 14
        </div>
      </div>
      <div
        ...
        >
          <div
            style={{
              ...
              height: 100,
              borderRadius: "50%",
              background: `radial-gradient(circle,${T.cyanD},#0a0a0a)`,
          ...
          >
            ...
            />
            <span style={{ color: T.green, fontWeight: 700, fontSize: 12 }}>
              CANAL ACTIF
            </span>
          </div>
          <div
            ...
            {Array.from({ length: 60 }).map((_, i) => (
              <div
                ...
                style={{
                  flex: 1,
                  height: `${
                    20 + Math.sin(i * 0.3) * 40 + Math.cos(i * 0.7) * 20
                  }%`,
                  minHeight: 4,
                  background: `linear-gradient(to top,${T.purple},${T.cyan})`,
                  borderRadius: 2,
                  opacity: 0.8,
                }}
              />
          ...
          <Panel>
            ...
            {[
              ...
              >
                ...
                <div>
                  <div style={{ color: T.muted, fontSize: 11 }}>{r.label}</div>
                  <div
                    style={{ color: r.color, fontWeight: 700, fontSize: 13 }}
                  >
                    {r.val}
                  </div>
                </div>
...
function PageOutilage({ tools, setTools, accounts, me }) {
  ...
  const colorMap = {
    "Hack ATM": T.cyan,
    Disqueuse: T.purple,
    "Disqueuse Pro": T.purple,
    "Kit Drogue": T.green,
    "Kit Drogue Premium": T.green,
    "Outils Braquage": T.orange,
    "Matériel Divers": T.yellow,
  };
  ...
  const add = () => {
    ...
    setTools((p) => [
      ...p,
      {
        ...
        color: colorMap[form.name] || T.yellow,
        joueur: form.joueur,
        date: todayStr(),
        qte: Number(form.qte),
      },
    ]);
    setForm({ name: "", joueur: "", qte: 1 });
    setShowForm(false);
  };
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      >
        <div>
          ...
          <div style={{ color: T.muted, fontSize: 13 }}>
            Distribution et gestion des équipements spéciaux
          </div>
        </div>
        {me.isAdmin && (
          <Btn onClick={() => setShowForm((p) => !p)} color={T.purple}>
            + Distribuer Outil
          </Btn>
        )}
      </div>
      ...
      {showForm && (
        <Panel>
          ...
          >
            ...
            <Field label="Joueur">
              ...
              >
                <option value="">—</option>
                {membres.map((a) => (
                  <option key={a.id}>{a.nomRP}</option>
                ))}
              </Sel>
            </Field>
            <Field label="Quantité">
              <Inp
                ...
                value={form.qte}
                onChange={(e) =>
                  setForm((p) => ({ ...p, qte: e.target.value }))
                }
              />
            </Field>
          </div>
          <div style={{ marginTop: 12, display: "flex", gap: 8 }}>
            <Btn
              onClick={add}
              disabled={!form.name || !form.joueur}
              color={T.purple}
            >
              ✓ Distribuer
            </Btn>
            <Btn onClick={() => setShowForm(false)} color={T.muted} outline>
              Annuler
            </Btn>
          </div>
        </Panel>
      ...
      >
        {typeBase.map((n) => {
          const cnt = tools
            .filter((t) => t.name.startsWith(n.split(" ")[0]))
            .reduce((a, t) => a + t.qte, 0);
          return (
            <div
              key={n}
              style={{
            ...
            >
              <div
                style={{
                  ...
                  height: 40,
                  borderRadius: 10,
                  background: (colorMap[n] || T.yellow) + "22",
                  display: "flex",
                  alignItems: "center",
                  justifyContent: "center",
                  fontSize: 18,
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      >
        <div>
          ...
          <div style={{ color: T.muted, fontSize: 13 }}>
            Gestion et traçabilité des armes de l'organisation
          </div>
        </div>
        {me.isAdmin && (
          <Btn onClick={() => setShowForm((p) => !p)} color={T.orange}>
            + Attribuer Arme
          </Btn>
        )}
      </div>
      ...
      {showForm && (
        <Panel>
          ...
          <div style={{ marginTop: 12, display: "flex", gap: 8 }}>
            <Btn
              onClick={add}
              disabled={!form.name || !form.serie || !form.joueur}
              color={T.orange}
            >
              ✓ Attribuer
            </Btn>
          ...
          {weapons.map((w) => (
            ...
            >
              {me.isAdmin && (
                <button
                  onClick={() =>
                    setWeapons((p) => p.filter((x) => x.id !== w.id))
                  }
                  style={{
                    position: "absolute",
                    top: 12,
              ...
              <div style={{ color: T.muted, fontSize: 12, marginBottom: 2 }}>
                Série
              </div>
              <div
                style={{
...
function PageCompta({ compta, setCompta, fondsOrg, setFondsOrg, me }) {
  ...
  const add = () => {
    ...
    const entry = {
      ...
      par: form.par,
      date: todayStr(),
      heure: `${String(d.getHours()).padStart(2, "0")}:${String(
        d.getMinutes()
      ).padStart(2, "0")}`,
    };
    setCompta((p) => [entry, ...p]);
    ...
    setFondsOrg((p) =>
      form.type === "credit"
        ? p + Number(form.montant)
        : p - Number(form.montant)
    );
    setForm({ type: "credit", motif: "", montant: "", par: me.nomRP });
    setShowForm(false);
  };
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      >
        <div>
          ...
          <div style={{ color: T.muted, fontSize: 13 }}>
            Gestion financière de l'organisation — Admin uniquement
          </div>
        </div>
        {me.isAdmin && (
          <Btn onClick={() => setShowForm((p) => !p)} color={T.orange}>
          ...
          >
            ...
            <Field label="Responsable">
              <Inp
                value={form.par}
                onChange={(e) =>
                  setForm((p) => ({ ...p, par: e.target.value }))
                }
              />
            </Field>
            <Field label="Motif">
              <Inp
                ...
                placeholder="Ex: Braquage banque"
              />
            </Field>
          </div>
          <div style={{ marginTop: 12, display: "flex", gap: 8 }}>
            <Btn
              onClick={add}
              disabled={!form.motif || !form.montant}
              color={T.orange}
            >
              ✓ Enregistrer
            </Btn>
      ...
      >
        <Panel style={{ background: "linear-gradient(135deg,#0a2a0a,#111)" }}>
          <div style={{ color: T.muted, fontSize: 13, marginBottom: 4 }}>
            Fonds Total Organisation
          </div>
          <div
            style={{
              color: fondsOrg >= 0 ? T.green : T.red,
              fontWeight: 900,
              fontSize: 36,
            }}
          >
          ...
          >
            {fondsOrg >= 0 ? "📈 Solde positif" : "📉 Solde négatif"}
          </div>
        </Panel>
        <Panel>
          <div style={{ color: T.muted, fontSize: 13, marginBottom: 4 }}>
          ...
          <div style={{ color: T.green, fontWeight: 900, fontSize: 24 }}>
            +{fmt(gains)} $
          </div>
          <div style={{ color: T.muted, fontSize: 11, marginTop: 4 }}>
            {compta.filter((c) => c.type === "credit").length} dépôts
          </div>
        </Panel>
        <Panel>
          <div style={{ color: T.muted, fontSize: 13, marginBottom: 4 }}>
          ...
          <div style={{ color: T.red, fontWeight: 900, fontSize: 24 }}>
            -{fmt(retraits)} $
          </div>
          <div style={{ color: T.muted, fontSize: 11, marginTop: 4 }}>
            {compta.filter((c) => c.type === "debit").length} retraits
          </div>
        </Panel>
      </div>
            ...
            >
              <div
                style={{
                  ...
                  height: 36,
                  borderRadius: "50%",
                  background: c.type === "credit" ? T.greenD : T.redD,
                  display: "flex",
                  alignItems: "center",
                  justifyContent: "center",
                  color: c.type === "credit" ? T.green : T.red,
                  fontSize: 16,
                  flexShrink: 0,
                  fontWeight: 700,
                }}
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      >
        <div>
          ...
          <div style={{ color: T.muted, fontSize: 13 }}>
            Inventaire sécurisé — visible uniquement par vous et l'admin
          </div>
        </div>
        {me.isAdmin && (
      ...
      <Panel style={{ display: "flex", alignItems: "center", gap: 14 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ color: T.muted, fontSize: 13 }}>
            {myW.length} arme(s) — {myT.length} outil(s)
          </div>
        </div>
        <div
          style={{
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <div>
                    ...
                    <div
                      style={{ color: t.color, fontWeight: 700, fontSize: 12 }}
                    >
                      x{t.qte}
                    </div>
...
function PagePermissions({
      ...
      p.map((a) =>
        a.id === id
          ? {
              ...
              isAdmin: !a.isAdmin,
              perms: !a.isAdmin
                ? Object.fromEntries(PERM_LIST.map((pp) => [pp.id, true]))
                : Object.fromEntries(PERM_LIST.map((pp) => [pp.id, false])),
            }
          : a
      )
    );
  ...
  const gradeUp = (id) =>
    setAccounts((p) =>
      p.map((a) => {
        if (a.id !== id) return a;
        const i = GRADES.indexOf(a.grade);
        return i < GRADES.length - 1 ? { ...a, grade: GRADES[i + 1] } : a;
      })
    );
  const gradeDown = (id) =>
    setAccounts((p) =>
      p.map((a) => {
        if (a.id !== id) return a;
        const i = GRADES.indexOf(a.grade);
        return i > 0 ? { ...a, grade: GRADES[i - 1] } : a;
      })
    );

  const savePerms = () => {
    setAccounts((p) =>
      p.map((a) => (a.id === sel ? { ...a, perms: localPerms } : a))
    );
    setSaved(true);
  ...
  const rejectReq = (id) =>
    setPendingAccounts((p) => p.filter((x) => x.id !== id));
  const banAcc = (id) =>
    setAccounts((p) =>
      p.map((a) => (a.id === id ? { ...a, banned: true, isAdmin: false } : a))
    );
  const unbanAcc = (id) =>
    setAccounts((p) =>
      p.map((a) => (a.id === id ? { ...a, banned: false } : a))
    );
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      >
        <div>
          ...
          <div style={{ color: T.muted, fontSize: 13 }}>
            Gestion des accès, rôles et membres
          </div>
        </div>
        <button
          onClick={() => setRecrutementOuvert((p) => !p)}
          style={{
            background: recrutementOuvert ? T.greenD : T.redD,
            color: recrutementOuvert ? T.green : T.red,
            border: `1px solid ${recrutementOuvert ? T.green : T.red}44`,
            borderRadius: 8,
            padding: "8px 16px",
            cursor: "pointer",
            fontWeight: 700,
      ...
      {tab === "demandes" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {pendingAccounts.length === 0 ? (
            <Panel>
              <Empty icon="📬" text="Aucune demande d'accès en attente." />
            </Panel>
          ) : (
            pendingAccounts.map((req) => (
              ...
              >
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <Btn onClick={() => approveReq(req)} color={T.green} small>
                    ✓ Accepter
                  </Btn>
                  <Btn
      ...
      {tab === "bans" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {bannis.length === 0 ? (
            <Panel>
              <Empty icon="🚫" text="Aucun compte banni." />
            </Panel>
          ) : (
            bannis.map((a) => (
              <Panel
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  <div
                    style={{ color: T.muted, fontWeight: 700, fontSize: 15 }}
                  >
                    {a.nomRP}
                  </div>
                  <div style={{ color: T.muted, fontSize: 12 }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <Btn onClick={() => unbanAcc(a.id)} color={T.green} small>
                    ♻ Réintégrer
                  </Btn>
                  <Btn onClick={() => deleteAcc(a.id)} color={T.red} small>
              ...
              membres.map((a) => (
                <button
                  ...
                  style={{
                    ...
                    gap: 10,
                    padding: "10px 12px",
                    background: sel === a.id ? T.orange + "22" : "transparent",
                    border: `1px solid ${sel === a.id ? T.orange : T.border}`,
                    borderRadius: 10,
                    cursor: "pointer",
                    width: "100%",
                    textAlign: "left",
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div
                      style={{
                        color: GRADE_COLORS[a.grade] || T.muted,
                        fontSize: 11,
                      }}
                    >
                      {a.grade}
            ...
            {!selFull ? (
              ...
              <Panel>
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    <div
                      style={{ color: T.text, fontWeight: 800, fontSize: 18 }}
                    >
                      {selFull.nomRP}
                    </div>
                    <div style={{ color: T.muted, fontSize: 13 }}>
                  ...
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    <Btn
                      onClick={() => toggleAdmin(selFull.id)}
                      color={selFull.isAdmin ? T.red : T.orange}
                      small
                    >
                      {selFull.isAdmin ? "❌ Retirer Admin" : "⭐ Nommer Admin"}
                    </Btn>
                    <Btn
                      onClick={() => banAcc(selFull.id)}
                      color={T.red}
                      outline
                      small
                    >
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    <span
                      style={{
                        ...
                        borderRadius: 20,
                        background:
                          (GRADE_COLORS[selFull.grade] || T.muted) + "22",
                        color: GRADE_COLORS[selFull.grade] || T.muted,
                        fontWeight: 700,
                        fontSize: 14,
                        border: `1px solid ${
                          GRADE_COLORS[selFull.grade] || T.muted
                        }44`,
                      }}
                    >
                      {selFull.grade}
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    <Btn
                      onClick={() => gradeDown(selFull.id)}
                      color={T.red}
                      small
                      outline
                      disabled={GRADES.indexOf(selFull.grade) === 0}
                    >
                      ▼ Rétrograder
                    </Btn>
                    <Btn
                      onClick={() => gradeUp(selFull.id)}
                      color={T.green}
                      small
                      disabled={
                        GRADES.indexOf(selFull.grade) === GRADES.length - 1
                      }
                    >
                      ▲ Promouvoir
                    </Btn>
                ...
                {selFull.isAdmin && (
                  ...
                  >
                    <span style={{ color: T.orange }}>🛡</span>
                    <span
                      style={{ color: T.orange, fontWeight: 700, fontSize: 13 }}
                    >
                      Administrateur — Toutes permissions actives
                    </span>
                  </div>
                )}
                    ...
                    return (
                      <button
                        ...
                        style={{
                          ...
                          gap: 12,
                          padding: "14px 16px",
                          background: on ? p.color + "11" : "#0f0f0f",
                          border: `1px solid ${on ? p.color + "44" : T.border}`,
                          borderRadius: 10,
                          cursor: selFull.isAdmin ? "default" : "pointer",
                          textAlign: "left",
                        }}
                      >
                        <div
                ...
                <Btn onClick={savePerms} color={saved ? T.green : T.orange}>
                  {saved ? "✓ Sauvegardé !" : "💾 Sauvegarder les permissions"}
                </Btn>
              </Panel>
            )}
          </div>
...
export default function App() {
  ...
  const renderPage = () => {
    switch (page) {
      ...
      case "cotas":
        return <PageCotas cotas={cotas} setCotas={setCotas} me={meSync} />;
      case "classement":
        return <PageClassement accounts={accounts} cotas={cotas} />;
      case "clips":
        return <PageClips clips={clips} setClips={setClips} me={meSync} />;
      case "rapport":
        return (
          <PageRapport
            rapports={rapports}
      ...
      case "radio":
        return <PageRadio radio={radio} setRadio={setRadio} me={meSync} />;
      case "outilage":
        return (
          <PageOutilage
            tools={tools}
      ...
      case "permissions":
        return (
          <PagePermissions
            ...
            setPendingAccounts={setPendingAccounts}
            recrutementOuvert={recrutementOuvert}
            setRecrutementOuvert={setRecrutementOuvert}
          />
        );
      default:
        return null;
  ...
  return (
    ...
    >
      <Sidebar
        ...
        unreadRapports={unreadRapports}
        isAdmin={meSync?.isAdmin || false}
      />
      <div
        style={{
          flex: 1,
